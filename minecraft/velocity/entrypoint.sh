...
if [ ${exit_result} == 0 ]; then
    ...
    adduser -g "Velocity server user" -h "/app" -s "/bin/bash" -D -u ${UID} --ingroup ${GROUP} velocity
else
    echo "GID does not exist, creating group..."
    addgroup -g ${GID} velocity
    adduser -g "Velocity server user" -h "/app" -s "/bin/bash" -D -u ${UID} --ingroup velocity velocity
