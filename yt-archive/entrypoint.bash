...
runChannels()
{
    ...
    do
        ...
        if ! test -f "/tmp/pids/pid.${n}"; then
        {
            ...
            LC_ALL=en_US.UTF-8 python3 /yt_dlp/__main__.py ${quiet_mode} ${archive_mode} ${cookies_enabled} ${DATE} -f ${FORMAT} -ciw -o "/data/${channelName}/${NAMING_CONVENTION}" ${channelUrl} &
            echo $! > "/tmp/pids/pid.${n}"
        }
        else
...
if [ "${ARCHIVE_DOWNLOADED}" == "true" ] || [ "${ARCHIVE_DOWNLOADED}" == "TRUE" ];
then
    archive_mode="--download-archive /tmp/.downloaded";
else
    archive_mode="";
fi
